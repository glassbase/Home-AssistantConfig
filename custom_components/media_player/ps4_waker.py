"""Playstation 4 media_player using ps4-waker.""" import json import re import subprocess import logging import urllib.request from 
datetime import timedelta from urllib.parse import urlparse import voluptuous as vol import homeassistant.util as util from 
homeassistant.components.media_player import (
    PLATFORM_SCHEMA,
    MEDIA_TYPE_CHANNEL,
    SUPPORT_TURN_ON,
    SUPPORT_TURN_OFF,
    ...
    ENTITY_IMAGE_URL,
    MediaPlayerDevice ) from homeassistant.const import (
    STATE_IDLE,
    STATE_UNKNOWN,
    STATE_OFF,
    STATE_PLAYING,
    ...
    CONF_HOST,
    CONF_PORT,
    CONF_FILENAME ) from homeassistant.helpers import config_validation as cv REQUIREMENTS = [] _LOGGER = logging.getLogger(__name__) 
SUPPORT_PS4 = SUPPORT_TURN_OFF | SUPPORT_TURN_ON | \
    SUPPORT_STOP | SUPPORT_SELECT_SOURCE DEFAULT_NAME = 'Playstation 4' DEFAULT_PORT = '' ICON = 'mdi:playstation' CONF_GAMES_FILENAME 
= 'games_filename' CONF_IMAGEMAP_JSON = 'imagemap_json' CONF_CMD = 'cmd' CONF_LOCAL_STORE = "local_store" PS4WAKER_CMD = 'ps4-waker' 
PS4WAKER_CONFIG_FILE = '.ps4-wake.credentials.json' PS4_GAMES_FILE = 'ps4-games.json' MEDIA_IMAGE_DEFAULT = None MEDIA_IMAGEMAP_JSON = 
'https://github.com/hmn/ps4-imagemap/raw/master/games.json' LOCAL_STORE = None MIN_TIME_BETWEEN_SCANS = timedelta(seconds=10) 
MIN_TIME_BETWEEN_FORCED_SCANS = timedelta(seconds=1) PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    add_devices([PS4Device(name, ps4, gamesmap_json, local_store)], True) class PS4Device(MediaPlayerDevice):
    ...
    def ps_store_cover_art(self):
        ...
        try:
            url = 
'https://store.playstation.com/valkyrie-api/en/US/19/faceted-search/'+urllib.parse.quote(self._media_title.encode('utf-8'))
    ...
    def select_source(self, source):
        ...
        for titleid, game in self.ps4.games.items():
            if source == game:
                ...
                self.update() class PS4Waker(object):
